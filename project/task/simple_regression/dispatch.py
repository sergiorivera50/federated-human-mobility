...
from project.task.simple_regression.dataset import get_dataloader_generators
from project.task.simple_regression.models import get_net
from project.task.simple_regression.train_test import get_fed_eval_fn, test, train
...
def dispatch_train(
    ...
    **kwargs: Any,
    ...
    if train_structure is not None and train_structure.upper() == "REGRESSION":
...
def dispatch_data(cfg: DictConfig, **kwargs: Any) -> DataStructure | None:
    ...
    if client_model_and_data is not None and partition_dir is not None:
        ...
        if client_model_and_data.upper() == "REGRESSION_FNN":
            return (
                get_net,
